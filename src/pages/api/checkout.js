import { getSupabaseClıent } from '../serverUtils/serverSupabaseClient';
...
export default async function checkoutHandler(req, res) {
  if (req.method === 'POST') {
    const supabase = getSupabaseClıent();
    ...
    if (cartItemsFetchError) {
      console.log('cartItem fetch error')
    ...
    try {
      const session = await stripe.checkout.sessions.create({
        ...
        line_items,
        mode: 'payment',
        success_url: `http://${req.headers['host']}/cart/?success=true`,
        cancel_url: `http://${req.headers['host']}/cart/?cancel=true`,
        payment_intent_data: {
          metadata: {
            order_id,
          },
